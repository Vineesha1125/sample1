...
def get_text_model():
    ...
    if _text_model is None:
        _logger.info("Initializing Detoxify model...")
        _text_model = Detoxify('original')
    return _text_model


def get_illegal_classifier():
    global _illegal_classifier
    if _illegal_classifier is None:
        _logger.info("Initializing BART zero-shot classification model...")
        _illegal_classifier = pipeline(
            "zero-shot-classification",
            model="facebook/bart-large-mnli"
        )
    return _illegal_classifier
